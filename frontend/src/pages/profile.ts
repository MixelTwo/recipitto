...
export default function render()
{
	...
	const recipes = query_recipes({ author_id: user.v.data?.id }); // mock filtering

	const activeTab = $<"recipes" | "favorites" | "settings">("recipes");

	Layout([
		Div("profile-page", [
			$(user, u => u.isLoading && Spinner()),
			$(user, u => u.error && Div("profile-page__error", "Не удалось загрузить данные пользователя")),
			$(user, u => u.data && (() =>
			{
				const data = u.data;
				return [
					Div("profile-page__header", [
						Div("profile-page__avatar", data.avatar
							? initEl("img", "profile-page__avatar-img", undefined, (el: HTMLImageElement) =>
							{
								el.src = data.avatar!;
								el.alt = data.name;
							})
							: Div("profile-page__avatar-placeholder", data.name.slice(0, 1).toUpperCase())
						),
						Div("profile-page__info", [
							H1([], data.name),
							Div("profile-page__meta", [
								Span([], `Логин: ${data.login}`),
								Span([], `Дата регистрации: ${new Date(data.reg_date).toLocaleDateString("ru-RU")}`),
								Span([], `Роли: ${data.roles.join(", ")}`),
							]),
						]),
